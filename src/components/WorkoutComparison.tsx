          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center">
                <span className="text-xs font-bold text-blue-600">{w.similarity}%</span>
              </div>
              <div>
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
      <h2 className="font-semibold text-slate-900 mb-4 flex items-center gap-2">
        <Zap className="w-5 h-5 text-yellow-500" />
        Running Power
        <span className="text-xs font-normal text-slate-400 ml-auto">Estimated</span>
      </h2>

      <div className="grid grid-cols-2 gap-4">
        <div className="text-center">
          <p className="text-2xl font-bold text-slate-900">{power.avgPower}</p>
          <p className="text-xs text-slate-500">Avg Power (W)</p>
        </div>
        <div className="text-center">
          <p className="text-2xl font-bold text-slate-900">{power.normalizedPower}</p>
          <p className="text-xs text-slate-500">Norm Power (W)</p>
        </div>
        <div className="text-center">
          <p className="text-2xl font-bold text-slate-900">{power.powerPerKg}</p>
          <p className="text-xs text-slate-500">W/kg</p>
        </div>
        <div className="text-center">
          <p className="text-2xl font-bold text-slate-900">{power.efficiency}%</p>
...
export function EfficiencyMetricsCard({ workoutId }: { workoutId: number }) {
  ...
  if (hasPaceDecoupling) {
    ...
    if (pd < 3) {
      ...
      decouplingStatus = { label: 'Good', color: 'text-blue-600' };
    } else if (pd < 8) {
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
      ...
      <div className="space-y-4">
        {hasPaceDecoupling && (
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm text-slate-600">Pace Decoupling</span>
        ...
        {hasAerobicEfficiency && (
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-sm text-slate-600">Aerobic Efficiency</span>
...
export function WorkoutComparisonView({
  ...
  if (!comparison) {
    return (
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
        <p className="text-slate-500 text-center">Unable to compare workouts</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
      ...
      <div className="grid grid-cols-3 gap-4 mb-4 pb-4 border-b border-slate-100">
        <div />
        <div className="text-center">
          <Link href={`/workout/${comparison.workout1.id}`} className="text-sm font-medium text-blue-600 hover:text-blue-700">
            {comparison.workout1.name}
          </Link>
          <p className="text-xs text-slate-400">{formatDate(comparison.workout1.date)}</p>
        </div>
        <div className="text-center">
          <Link href={`/workout/${comparison.workout2.id}`} className="text-sm font-medium text-blue-600 hover:text-blue-700">
            {comparison.workout2.name}
          </Link>
          <p className="text-xs text-slate-400">{formatDate(comparison.workout2.date)}</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {comparison.differences.map((diff) => (
          <div key={diff.metric} className="grid grid-cols-3 gap-4 items-center">
            <span className="text-sm text-slate-600">{diff.label}</span>
            <div className="text-center">
              <span className={`text-sm font-mono ${diff.better === 1 ? 'text-green-600 font-semibold' : 'text-slate-700'}`}>
                {diff.metric === 'pace' ? formatPace(diff.value1 as number) : diff.value1}
              </span>
            </div>
            <div className="text-center flex items-center justify-center gap-1">
              <span className={`text-sm font-mono ${diff.better === 2 ? 'text-green-600 font-semibold' : 'text-slate-700'}`}>
                {diff.metric === 'pace' ? formatPace(diff.value2 as number) : diff.value2}
              </span>
              {diff.diff !== 0 && (
                <span className={`text-xs ${diff.better === 2 ? 'text-green-500' : diff.better === 1 ? 'text-red-500' : 'text-slate-400'}`}>
      ...
      {comparison.lapComparison && comparison.lapComparison.length > 0 && (
        <div className="mt-6 pt-6 border-t border-slate-100">
          <h3 className="text-sm font-medium text-slate-700 mb-3">Lap-by-Lap</h3>
          <div className="space-y-2">
            {comparison.lapComparison.slice(0, 10).map((lap) => (
              <div key={lap.lap} className="flex items-center gap-2 text-sm">
                <span className="w-12 text-slate-400">Lap {lap.lap}</span>
                <span className="w-16 font-mono text-slate-600">
                  {lap.pace1 ? formatPace(lap.pace1) : '-'}
                </span>
                <div className="flex-1 flex items-center justify-center">
                  {lap.diff !== null && (
                    <span className={`text-xs ${lap.diff < 0 ? 'text-green-500' : lap.diff > 0 ? 'text-red-500' : 'text-slate-400'}`}>
                ...
                <span className="w-16 font-mono text-slate-600 text-right">
