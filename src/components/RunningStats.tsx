  ...
  if (!streak || streak.streakStatus === 'no_data') {
    return (
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
        ...
        <p className="text-sm text-slate-500">Start running to build your streak!</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="text-center">
          <div className={`text-4xl font-bold ${streak.streakStatus === 'active' ? 'text-orange-500' : 'text-slate-400'}`}>
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-slate-500 mt-1">Longest</p>
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        <div className="text-center">
          <p className="text-2xl font-bold text-slate-900">{milestones.totalMilesAllTime}</p>
          <p className="text-xs text-slate-500">Miles</p>
        </div>
        <div className="text-center">
          <p className="text-2xl font-bold text-slate-900">{milestones.totalRunsAllTime}</p>
          <p className="text-xs text-slate-500">Runs</p>
        </div>
        <div className="text-center">
          <p className="text-2xl font-bold text-slate-900">{Math.round(milestones.totalHoursAllTime)}</p>
      ...
      <div className="space-y-2 pt-4 border-t border-slate-100">
        {milestones.longestRun && (
          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-500">Longest Run</span>
            <Link href={`/workout/${milestones.longestRun.id}`} className="text-blue-600 hover:text-blue-700">
        ...
        {milestones.fastestMile && (
          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-500">Fastest Pace</span>
            <Link href={`/workout/${milestones.fastestMile.id}`} className="text-blue-600 hover:text-blue-700">
        ...
        {milestones.mostElevation && (
          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-500">Most Climbing</span>
            <Link href={`/workout/${milestones.mostElevation.id}`} className="text-blue-600 hover:text-blue-700">
        ...
        {milestones.biggestWeek && (
          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-500">Biggest Week</span>
            <span className="text-slate-700">{milestones.biggestWeek.miles} mi</span>
          </div>
        )}
        {milestones.biggestMonth && (
          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-500">Biggest Month</span>
            <span className="text-slate-700">{milestones.biggestMonth.miles} mi ({milestones.biggestMonth.month})</span>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
        ...
        {data.days.map((day) => {
          ...
          return (
            <div key={day.day} className="flex-1 flex flex-col items-center">
              <div
                className={`w-full rounded-t transition-all ${isActive ? 'bg-blue-500' : 'bg-slate-300'}`}
                style={{ height: `${Math.max(height, 4)}%` }}
                title={`${day.day}: ${day.count} runs, ${day.miles} mi`}
              />
              <span className="text-xs text-slate-500 mt-1">{day.day.slice(0, 3)}</span>
      ...
      {data.mostActiveDay && (
        <p className="text-xs text-slate-500 mt-4 pt-4 border-t border-slate-100">
          You run most often on <span className="font-medium text-blue-600">{data.mostActiveDay}s</span>
        </p>
      )}
    </div>
  );
      ...
      <div className="space-y-2">
        {data.tempRanges.filter(r => r.count > 0).map((range) => (
          <div key={range.range} className="flex items-center gap-2 text-sm">
            <span className="w-28 text-slate-600 truncate">{range.range}</span>
            <div className="flex-1 h-2 bg-slate-100 rounded-full">
              <div
                className={`h-2 rounded-full ${range.range === data.optimalTemp ? 'bg-green-500' : 'bg-slate-400'}`}
            ...
            <span className="w-8 text-right text-slate-400">{range.count}</span>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {facts.slice(0, 6).map((fact, i) => (
          <div key={i} className="bg-slate-50 rounded-lg p-3">
            ...
            {fact.detail && (
              <p className="text-xs text-slate-400 mt-1">{fact.detail}</p>
            )}
          </div>
        ))}
      </div>
