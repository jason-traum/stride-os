...
export default async function PerformanceTrendsPage() {
  const [weekResult, monthResult, quarterResult] = await Promise.allSettled([
    analyzePerformanceTrends('week'),
    analyzePerformanceTrends('month'),
    analyzePerformanceTrends('quarter')
  ]);

  const weekTrends = weekResult.status === 'fulfilled' ? weekResult.value : null;
  const monthTrends = monthResult.status === 'fulfilled' ? monthResult.value : null;
  const quarterTrends = quarterResult.status === 'fulfilled' ? quarterResult.value : null;

  const hasData = (weekTrends?.metrics.workoutCount.current ?? 0) > 0 || (monthTrends?.metrics.workoutCount.current ?? 0) > 0;
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        {!hasData ? (
          <div className="bg-surface-1 rounded-xl border border-default p-8 text-center shadow-sm">
            <TrendingUp className="w-12 h-12 text-textTertiary mx-auto mb-3" />
            <h3 className="text-lg font-medium text-primary mb-2">No workout data yet</h3>
            <p className="text-textSecondary text-sm">
              Log some runs to see your performance trends over time.
            </p>
          </div>
        ) : (
          <>
