    ...
    return (
      <div className="min-h-screen bg-stone-50 p-4">
        <div className="mx-auto max-w-4xl">
          <h1 className="text-3xl font-bold text-stone-900 mb-8">Best Efforts</h1>
          <div className="bg-white rounded-xl border border-stone-200 p-8 text-center">
            <AlertCircle className="w-12 h-12 text-stone-300 mx-auto mb-3" />
            <p className="text-stone-500">Unable to load best efforts. Please try again.</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      <div className="mx-auto max-w-6xl p-4 sm:p-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-stone-900 mb-2">Best Efforts</h1>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {analysis.recentPRs.slice(0, 6).map((pr, idx) => (
                <Link
                  ...
                  className="bg-white rounded-lg border border-stone-200 p-4 hover:border-teal-300 hover:shadow-sm transition-all"
                >
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <p className="font-semibold text-stone-900">{pr.distance}</p>
        ...
        <div>
          <h2 className="text-xl font-semibold text-stone-900 mb-4">All-Time Records</h2>

          {sortedDistances.length === 0 ? (
            <div className="bg-white rounded-xl border border-stone-200 p-8 text-center">
              <Trophy className="w-12 h-12 text-stone-300 mx-auto mb-3" />
              <p className="text-stone-500 mb-2">No efforts detected yet</p>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              {sortedDistances.map(distance => {
                ...
                return (
                  <div key={distance} className="bg-white rounded-xl border border-stone-200 overflow-hidden">
                    {/* Distance Header */}
                    <div className="bg-gradient-to-r from-slate-50 to-teal-50 p-4 border-b border-stone-200">
                      <div className="flex items-center justify-between">
                        <h3 className="text-lg font-semibold text-stone-900">{distance}</h3>
                        {best.equivalentVDOT && (
                          <span className="text-sm text-stone-500">VDOT {best.equivalentVDOT}</span>
                        )}
                      </div>
                      <div className="mt-2">
                        <p className="text-2xl font-bold text-teal-600">{best.timeFormatted}</p>
                        <p className="text-sm text-stone-600">{best.pace}/mi • {new Date(best.workoutDate).toLocaleDateString()}</p>
                      </div>
                    </div>
                    ...
                    {topEfforts.length > 1 && (
                      <div className="p-4">
                        <p className="text-xs font-medium text-stone-500 uppercase tracking-wide mb-2">Top Performances</p>
                        <div className="space-y-2">
                          {topEfforts.map((effort, idx) => (
                            <Link
                              ...
                              className={cn(
                                'flex items-center justify-between p-2 rounded-lg hover:bg-stone-50 transition-colors text-sm',
                                idx === 0 && 'bg-yellow-50'
                              )}
                            >
                              <div className="flex items-center gap-3">
                                <span className={cn(
                                  'font-medium',
                                  idx === 0 ? 'text-yellow-600' : 'text-stone-500'
                                )}>
                                  #{effort.rankAllTime}
                                </span>
                                <span className="font-medium text-stone-900">{effort.timeFormatted}</span>
                              </div>
                              <span className="text-stone-500">{new Date(effort.workoutDate).toLocaleDateString()}</span>
                            </Link>
                          ))}
                        </div>
                      </div>
