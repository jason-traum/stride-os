...
import { TimeRangeSelector as SharedTimeRangeSelector, TIME_RANGES_EXTENDED } from '@/components/shared/TimeRangeSelector';
import { Vo2maxTimelineChart } from '@/components/charts/Vo2maxTimelineChart';
import { EfficiencyFactorChart } from '@/components/charts/EfficiencyFactorChart';
...
export default function PredictionsPage() {
  ...
  useEffect(() => {
    async function load() {
      const actionResult = await getPredictionDashboardData();
      if (actionResult.success) {
        setData(actionResult.data.data);
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      {/* VO2max Timeline */}
      <Vo2maxTimelineChart signalTimeline={signalTimeline} blendedVdot={prediction.vdot} />

      {/* Efficiency Factor Trend */}
      <EfficiencyFactorChart signalTimeline={signalTimeline} />

      {/* Pace vs Heart Rate Scatter */}
      <PaceHrScatterChart signalTimeline={signalTimeline} />
...
function RacePredictionTrends({ vdotHistory }: { vdotHistory: VdotHistoryEntry[] }) {
  ...
  const [selectedDists, setSelectedDists] = useState<Set<DistKey>>(() => new Set(['5k', 'half', 'marathon'] as DistKey[]));
